...
The most recent and related work is from Patni et al., they explored the potential and limitations of using standard robotic grippers to estimate the elasiticity and viscoelasticity of deformable materials. [[15]](#15) However, their focus is on using single grip to classify the object, but not giving a delicate modeling of the object property.

#### Terminologies that might worth looking at. (Generated by ChatGPT's deep research functionality, possibility of illusion is not zero)
Coming to this field, we might notice that not all things are clearly defined and given a name. Even if some are given a name, there might exist some two of the terminologies pointing to one thing (or two things with trivial difference.) Here I present a list of terminologies in thise field. 
- Deformable object property estimation (deformable object modeling)
- Material parameter identification (material property inference)
- Visuo-haptic sensing or multimodal perception for object properties
- Active tactile exploration (active palpation, poke testing)
- Instance-level modeling (in constrast, category-level modeling of material properties)
- Online parameter estimation  or online system identification (for deformables)
- Physics-based modeling (e.g. FEM(Finite Element Method) parameter calibration, mass-spring model tuning)
- Deep learning for material properties (e.g. tactile deep networks, visual material recognition)
- Reinforcement learning for active sensing (optimal actions to discern properties)
- Application-specific terms: robotic palpation(surgical), fruit firmness estimation(agricultural), tactile material classification.

Out of all the terminologies, active tactile exploration should be the one that we are going after. Though not trying to model one object precisely, we would like to design a mechanism that enables safe manipulation of delicate objects. In other words, we don't need to know the exact property of one object, but we would like to know how to interact with them without breaking them. 

## Structure of this repository and naming conventions
Now talking about this repository, we will put all the framework into separate bins: each setup will get their own bin. Naming convention for each setup will be LOCATION_CAMERA_HAND_INDEX. One setup should be linked to the corresponding software packages that support its camera/gripper/arm. 

### Code Hierarchy 
```THIS IS RERSERVED FOR A TREE STRUCTURE FOR THE CODE```
